...
- (void)viewDidLoad {
    ...
    Previous.layer.cornerRadius = 8.0;
    Previous.frame = CGRectMake(70, 512, 45, 45);
    [Previous setImage:[UIImage imageNamed:@"previous.png"] forState:(UIControlStateNormal)];
    
    //[Next setTitle:@"查询" forState:(UIControlStateNormal)];
    //[Next setBackgroundColor:[UIColor greenColor]];
    [Previous setTintColor:[UIColor darkGrayColor]];
    [self.view addSubview:Previous];
    [Previous addTarget:self action:@selector(previous_click) forControlEvents:(UIControlEventTouchUpInside)];
    
    UIButton *Finish = [UIButton buttonWithType:(UIButtonTypeRoundedRect)];
    Finish.layer.cornerRadius = 8.0;
    Finish.frame = CGRectMake(220, 512, 120, 45);
    [Finish setTitle:@"添加" forState:(UIControlStateNormal)];
    Finish.titleLabel.font = [UIFont fontWithName:@"Helvetica-Bold" size:18];//设置按钮的标题字体类型及大小
    [Finish setBackgroundColor:[UIColor greenColor]];
    [Finish setTintColor:[UIColor whiteColor]];
...
-(void)insertSql
{
    ...
    //新增员工个人各项工资的插入sql语句
//    NSString*salaryInserSql=[NSString stringWithFormat:@"insert into wages(e_name, basepay,welfare,rewards,claim,hfound,r_wages) values ('%@','%@','%@','%@','%@','%@','%@')",_inputName,inputBaseSalary.text,inputWelfare.text,inputBonusWage.text,inputUnemployInsurance.text,inputHousingFund.text,[NSString stringWithFormat:@"%f",[self salarycalculation]]];
